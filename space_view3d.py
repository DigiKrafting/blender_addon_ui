...
class VIEW3D_HT_header(Header):
    ...
    def draw(self, context):
        ...
        global icons, preview_collections
        ...
        sub.label(text="",icon='THREE_DOTS')
        ...
        if obj:
            ...
            if obj.type == 'MESH':
                ...
                pe = tool_settings.particle_edit

                if pe:

                    ob = pe.object

                    if ob and pe.type == 'PARTICLES':
            ...
            if obj.type == 'ARMATURE':
                ...
                sub.operator("object.mode_set",text="",icon='EDITMODE_HLT').mode="EDIT"
                sub.operator("object.mode_set",text="",icon='POSE_HLT').mode="POSE"
        ...
        if mode_string == 'OBJECT':
            ...
            row.operator("object.select_all",text="",icon='RESTRICT_SELECT_OFF').action="SELECT"
            row.operator("object.select_all",text="",icon='RESTRICT_SELECT_ON').action="DESELECT"
            row.operator("object.select_all",text="",icon='ARROW_LEFTRIGHT').action="INVERT"

            row.label(text="",icon='THREE_DOTS')

        elif mode_string == 'EDIT_MESH':

            row.operator("mesh.select_all",text="",icon='RESTRICT_SELECT_OFF').action="SELECT"
            row.operator("mesh.select_all",text="",icon='RESTRICT_SELECT_ON').action="DESELECT"
            row.operator("mesh.select_all",text="",icon='ARROW_LEFTRIGHT').action="INVERT"

            row.label(text="",icon='THREE_DOTS')

        elif mode_string == 'POSE':

            row.operator("pose.select_all",text="",icon='RESTRICT_SELECT_OFF').action="SELECT"
            row.operator("pose.select_all",text="",icon='RESTRICT_SELECT_ON').action="DESELECT"
            row.operator("pose.select_all",text="",icon='ARROW_LEFTRIGHT').action="INVERT"

            row.label(text="",icon='THREE_DOTS')

        if mode_string == 'EDIT_MESH':

            row.operator("mesh.select_edge_loop_multi",text="",icon='EDGESEL')
            row.operator("mesh.select_edge_ring_multi",text="",icon='SNAP_EDGE')
        ...
        if mode_string == 'OBJECT':
            ...
            if len(context.selected_objects) > 0:
                ...
                if len(context.selected_objects) > 1:
                    ...
                    row.operator("object.join",text="",icon='MOD_SOLIDIFY')

                row.operator("object.duplicate_move",text="",icon='MOD_PHYSICS')
                row.operator("object.delete",text="",icon='X').use_global=False

                layout.label(text="",icon='THREE_DOTS')

                row = layout.row(align=True)
                row.operator("view3d.localview",text="",icon='HIDE_OFF')
                row.operator("dks_modeling.object_center",text="",icon='PIVOT_CURSOR')
                row.operator("dks_modeling.object_transform_apply",text="",icon='OBJECT_ORIGIN')
                row.operator("dks_modeling.object_origin_geometry",text="",icon='ORIENTATION_GIMBAL')
        ...
        if mode_string == 'OBJECT' or mode_string == 'EDIT_MESH' or mode_string == 'EDIT_ARMATURE' or mode_string == 'POSE' or  mode_string == 'PAINT_WEIGHT':

            row = layout.row(align=True)
            row.operator("view3d.view_axis", text="",icon_value=icons["DKS_VIEW_FRONT"].icon_id).type = 'FRONT'
            row.operator("view3d.view_axis", text="",icon_value=icons["DKS_VIEW_BACK"].icon_id).type = 'BACK'
            row.operator("view3d.view_axis", text="",icon_value=icons["DKS_VIEW_LEFT"].icon_id).type = 'RIGHT'
            row.operator("view3d.view_axis", text="",icon_value=icons["DKS_VIEW_RIGHT"].icon_id).type = 'LEFT'
            row.operator("view3d.view_axis", text="",icon_value=icons["DKS_VIEW_TOP"].icon_id).type = 'TOP'
            row.operator("view3d.view_axis", text="",icon_value=icons["DKS_VIEW_BOTTOM"].icon_id).type = 'BOTTOM'
        ...
        if mode_string == 'OBJECT':
            ...
            if len(context.selected_objects) > 0:
                ...
                if bpy.context.preferences.addons[__package__].preferences.option_ui_mode == "Animation":

                    row.operator("dks_rigging.insert_keyframe_location", text="Loc", icon='KEYINGSET')
                    row.operator("dks_rigging.insert_keyframe_rotation", text="Rot", icon='KEYINGSET')
                    row.operator("dks_rigging.insert_keyframe_location_rotation", text="LocRot", icon='KEYINGSET')
                    row.label(text="",icon='THREE_DOTS')
            ...
            row = layout.row(align=True)
            row.operator("mesh.mark_seam",text="+ Seam").clear=False
            row.operator("mesh.mark_seam",text="- Seam").clear=True
            row.operator("mesh.mark_sharp",text="+ Sharp").clear=False
            row.operator("mesh.mark_sharp",text="- Sharp").clear=True

            row = layout.row(align=True)
            row.operator("mesh.edge_face_add",text='',icon='GRID')
#,description="Make Edge/Face"
            row.operator("dks_modeling.object_normals_reset",text='',icon='HANDLE_VECTOR')
#,text="Reset Vectors"
            # bpy.ops.mesh.normals_tools(mode='RESET')

            row.operator("dks_modeling.object_remove_doubles",text="",icon='SURFACE_NCYLINDER')

            row.operator("mesh.delete_loose",text='',icon='GHOST_ENABLED')
            ...
            row = layout.row(align=True)
            row.operator("dks_rigging.weight_paint_auto_weight",icon='BRUSH_DATA')
            row.menu("MENU_MT_vertex_groups", icon="COLLAPSEMENU", text=" Vertex Groups")
            row.label(text="",icon='THREE_DOTS')

        elif mode_string == 'EDIT_ARMATURE' or mode_string == 'POSE':

            row = layout.row(align=True)

            if mode_string == 'POSE' and bpy.context.preferences.addons[__package__].preferences.option_ui_mode == "Animation":

                bone_selection = bpy.context.selected_pose_bones_from_active_object

                if len(bone_selection) > 0:
            ...
            row.menu("MENU_MT_armatures", icon="COLLAPSEMENU", text=" Armatures")
            row.menu("MENU_MT_armature_poses", icon="COLLAPSEMENU", text=" Poses")
            row.menu("MENU_MT_armature_bones", icon="COLLAPSEMENU", text=" Deform Bones")
            row.menu("MENU_MT_armature_bones_ik", icon="COLLAPSEMENU", text=" IK Bones")
            row.label(text="",icon='THREE_DOTS')

            #dks_rigging.menu(context, layout)
